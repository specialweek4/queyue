...
import com.specialweek.shop.mapper.ShopTypeMapper;
...
import static com.specialweek.common.util.RedisConstants.CACHE_SHOP_TYPE_KEY;
...
public class ShopTypeServiceImpl extends ServiceImpl<ShopTypeMapper, ShopType> implements IShopTypeService {
    ...
    public Result queryTypeList() {
        ...
        if (CollectionUtil.isNotEmpty(shopTypeJson)) {
            //从 Redis 中获取 JSON 字符串列表
            // 利用 List 的 toString 方法将其拼装成类 JSON 数组格式的字符串
            // 再结合 ShopType 类型标识，解析为对应的对象集合。
        ...
        // 5. 存在， 写入Redis，这里使用Redis的List类型。使用String类型的话，就是直接所有都写在一起，对内存开销比较大。
